...
{
    /// <summary>
    /// Funcionalidad para una nueva actividad extraescolar
    /// </summary>
    public partial class NuevaActividad : PageBase
    {
        /// <summary>
        ...
        protected void BtnAceptar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (ValidarFormulario())
                {
                    ...
                    #endregion
                    if (NegActividad.AltaActividad(actividad, actHorario, actDescuento) > 0)
                    {
                        VaciarFormulario(this);
                        Session["listadoHorario"] = null;
                        Session["listadoDescuento"] = null;
                        PanelInfo.MostrarMensaje(Comun.TipoDatos.TipoError.Success, "La actividad extraescolar ha sido dado de alta correctamente");
                    }
                    else
                        PanelInfo.MostrarMensaje(Comun.TipoDatos.TipoError.Error, "Se ha producido un error al dar de alta la actividad extraescolar");
                }
                else
            ...
            {
                Comun.Log.TrazaLog.Error("Error al insertar la actividad extraescolar.", ex);
                PanelInfo.MostrarMensaje(Comun.TipoDatos.TipoError.Error, "Se ha producido un error al dar de alta la actividad extraescolar");
            }
        }
        ...
        {
            gvDescuento.DataSource = null;
            gvDescuento.DataBind();

            gvHorarios.DataSource = null;
            gvHorarios.DataBind();

            txtHoraIni.Value = string.Empty;
            txtHoraFin.Value = string.Empty;
